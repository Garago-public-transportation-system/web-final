...
Generated diagrams live in the repository root: [`erd.svg`](erd.svg) for the
entity-relationship model, [`class_diagram.svg`](class_diagram.svg) for the class
structure, and [`diagrams.md`](diagrams.md) for the Mermaid sources.
...
| Frontend | React 18, Vite, MUI, Zustand, Recharts | Operator console with charts, RTL support, and role-specific dashboards |
...
Interactive OpenAPI documentation is served at `/docs` once the app is running.
...
[`clean_dump.sql`](clean_dump.sql) contains a synthetic dataset — fictional drivers,
routes, vehicles, and trips — for exploring the system. It contains no real personal
data.

```bash
psql -d smart_bus_garage -f clean_dump.sql
```

For a larger generated fixture, `seed_exhaustive_matrix.py` builds an exhaustive
combination matrix. It targets whatever `TEST_DATABASE_URL` points at and **wipes it**,
...
documents/        full project write-up (markdown + PDF renders)
test_db_csv/      synthetic fixture data
...
The full write-up is in [`documents/`](documents/), readable directly on GitHub:

| Chapter | |
|---|---|
| [Title page and abstract](documents/Chapter_0_Title_Page_and_Abstract.md) | |
| [Introduction](documents/Chapter_1_Introduction.md) | Problem and scope |
| [Background and literature review](documents/Chapter_2_Background_and_Literature_Review.md) | Prior systems |
| [System planning and analysis](documents/Chapter_3_System_Planning_and_Analysis.md) | Requirements |
| [System architecture and design](documents/Chapter_4_System_Architecture_and_Design.md) | The design |
| [System implementation](documents/Chapter_5_System_Implementation.md) | How it was built |
| [Communication protocols](documents/Chapter_6_Communication_Protocols.md) | REST, WebSocket, hardware |
| [System evaluation and testing](documents/Chapter_7_System_Evaluation_and_Testing.md) | Results |
| [Parking reservation market](documents/Chapter_8_Parking_Reservation_Market.md) | Extension study |
| [Conclusion and future work](documents/Chapter_9_Conclusion_and_Future_Work.md) | |
| [References](documents/Chapter_10_References.md) | |

Additional material: the [API reference](documents/API_REFERENCE_v3.0.pdf), the
[plate-recognition chapter](documents/Chapter_7_Plate_Recognition_with_AI.pdf), the
[hardware integration guide](documents/HARDWARE_INTEGRATION_GUIDE.pdf), and a set of
`Q*.pdf` deep-dives on individual backend concerns.

> Chapter numbering is inconsistent in places — the write-up was reorganised late and
> both the original and revised numbering survive on disk. The markdown files are the
> current source; the PDFs are earlier renders.
