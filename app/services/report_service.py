...
from app.models.models import Trip, DailyReport, Driver, Vehicle, TripStatus
...
async def generate_daily_report(
    ...
    # Aggregation query using portable case() expressions
    stmt = select(
        func.count(Trip.id).label("total"),
        func.sum(case((Trip.status == TripStatus.COMPLETED, 1), else_=0)).label("completed"),
        func.sum(case((Trip.status == TripStatus.CANCELLED, 1), else_=0)).label("cancelled"),
        ...
        func.avg(case((Trip.is_late == False, 1.0), else_=0.0)).label("on_time")
...
async def get_dynamic_report(
    ...
    # Calculate summary
    total_revenue = sum(t.fare_collected or 0 for t in trips)
