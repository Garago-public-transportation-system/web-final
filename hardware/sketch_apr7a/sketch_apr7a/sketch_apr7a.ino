...
bool shouldClose(bool isOpen, unsigned long openedAt, unsigned long lastSeen, uint8_t clearStreak) {
  if (!isOpen) return false;
  unsigned long now = millis();
  if (now - openedAt < GATE_MIN_OPEN_MS) return false;
  if (now - openedAt > GATE_MAX_OPEN_MS) return true;
  if (clearStreak < CLEAR_CONFIRM_N) return false;
...
void showLCD() {
  ...
  if (free < 0) free = 0;
  lcd.clear();
  lcd.setCursor(0, 0); lcd.print("Free Slots:");
  lcd.setCursor(0, 1); lcd.print(free);
}
...
void ensureWiFiConnected() {
  ...
  if (WiFi.status() == WL_CONNECTED) sendLog("WiFi reconnected");
}
...
void entryGateTask(void* /*param*/) {
  if (carCount >= MAX_CARS) {
    ...
    if (triggerCamera(ENTRY_CAM_URL, "CAM_ENTRY")) {
      setGateIn(true);
      gateInOpen = true; gateInOpenTime = millis();
...
void exitGateTask(void* /*param*/) {
  if (carCount <= 0) {
    ...
    if (triggerCamera(EXIT_CAM_URL, "CAM_EXIT")) {
      setGateOut(true);
      gateOutOpen = true; gateOutOpenTime = millis();
...
void setup() {
  ...
  pinMode(LED_ENTRY, OUTPUT); pinMode(LED_FULL, OUTPUT); pinMode(LED_EXIT, OUTPUT);
  digitalWrite(LED_ENTRY, LOW); digitalWrite(LED_FULL, LOW); digitalWrite(LED_EXIT, LOW);

  Wire.begin(21, 22);
  lcd.init(); lcd.backlight();
  lcd.setCursor(0, 0); lcd.print("Garago Starting");

  ledcAttach(SERVO_IN,  pwmFreq, pwmResolution);
  ledcAttach(SERVO_OUT, pwmFreq, pwmResolution);
  setGateIn(false); setGateOut(false);
...
void loop() {
  ...
  if (dIn < DETECT_CM) {
    if (inDetectStreak < 255) inDetectStreak++;
    inClearStreak = 0;
    gateInLastSeen = millis();
  } else if (dIn > CLEAR_CM) {
    if (inClearStreak < 255) inClearStreak++;
    inDetectStreak = 0;
  }

  if (dOut < DETECT_CM) {
    if (outDetectStreak < 255) outDetectStreak++;
    outClearStreak = 0;
    gateOutLastSeen = millis();
  } else if (dOut > CLEAR_CM) {
    if (outClearStreak < 255) outClearStreak++;
    outDetectStreak = 0;
  }

  // Close: sensor-driven, not time-driven
  if (shouldClose(gateInOpen, gateInOpenTime, gateInLastSeen, inClearStreak)) {
    setGateIn(false); gateInOpen = false;
    carCount++;
  ...
  if (shouldClose(gateOutOpen, gateOutOpenTime, gateOutLastSeen, outClearStreak)) {
    setGateOut(false); gateOutOpen = false;
  ...
  if (inDetectStreak >= DETECT_CONFIRM_N && !inDetected && !entryBusy && !gateInOpen) {
    inDetected = true; entryBusy = true;
    xTaskCreate(entryGateTask, "entry_gate", 8192, NULL, 1, NULL);
  }
  if (inClearStreak >= CLEAR_CONFIRM_N) inDetected = false;

  if (outDetectStreak >= DETECT_CONFIRM_N && !outDetected && !exitBusy && !gateOutOpen) {
    outDetected = true; exitBusy = true;
    xTaskCreate(exitGateTask, "exit_gate", 8192, NULL, 1, NULL);
  }
  if (outClearStreak >= CLEAR_CONFIRM_N) outDetected = false;
