/*
 * Garago — ESP32-CAM Entry Gate Camera
 * Board: AI Thinker ESP32-CAM
 * Backend: local network (HTTP)
 */
...
#define UPLOAD_URL  BACKEND_BASE "/anpr/upload_raw?gate_id=1"
...
void sendLog(const String& msg) {
  ...
  String json = "{\"device\":\"CAM_ENTRY\",\"msg\":\"" + msg + "\"}";
  http.POST(json);
  http.end();
}
...
// ─── Capture & upload to backend (HTTP) ──────────────────────────────────────
void handleCapture() {
...
void setup() {
  ...
  server.begin();
  Serial.println("ENTRY CAMERA READY — waiting for /capture");
  sendLog("Entry camera ready");
}
